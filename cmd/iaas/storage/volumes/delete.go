...
var deleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		fmt.Fprintf(&summary, "Are you sure you want to delete the following volume(s)?\n")
		...
		for _, volume := range volumesToDelete {
			...
			if wait {
				...
				defer cancel()
				if err := client.IaaS().WaitUntilVolumeIsDeleted(waitCtx, volume.Identity); err != nil {
...
func init() {
	...
	deleteCmd.Flags().DurationVar(&waitTimeout, "wait-timeout", 20*time.Minute, "Maximum time to wait for the volume(s) to be deleted")
